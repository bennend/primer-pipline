# This code is also available on Github: https://github.com/bennend/primer-pipline
import os
...
# Code for the frame for the pipeline:
...
# Create the 4 pages that are going to be used and create a show_frame function to switch between different pages:
        for F in (WelcomePage, Implement_MSA, Generate_Consensus_Find_Conserved_Region, Design_Primers):
            frame = F(container, self)
            self.frames[F] = frame
# Make sure that the next page can cover the entire former page when switching them:
            frame.grid(row=0, column=0, sticky="nsew")
        self.show_frame(WelcomePage)
# Create a function to show frames created before:
...
# Setting the Font for the texts that are going to be shown on GUI of the pipeline:
...
# Create wigets that of the Welcome page:
        self.label1=tk.Label(self)
        self.label1.pack(side=BOTTOM)
# Create label to show page title:
        self.label = tk.Label(self, text="Welcome to PrimerDesigner!",font=self.__class__.appTitleFont)
        self.label.pack(side=TOP)
# Call the show_frame function to go to 'Design primers' page when click the 'Design primers' button:
...
# Call the show_frame function to go to 'Detect conserved regions' page when click the 'Design primers' button:
...
# Call the show_frame function to go to 'Implement MSA' page when click the 'Design primers' button:
...
# Call the broswer function when click 'Browse' button:
        ...
        self.label5=tk.Label(self,text='Create your alignment file',font=WelcomePage.appNormalFont)
        self.label5.pack()
...
# Call Clustal W2 and run multiple sequence alignment with the chosen file:
...
# Create textbox for showing information on GUI:
...
# Function call Clustal W (in the same directory of this code)and run it:
...
# Generate_Consensus_Find_Conserved_Region page:
class Generate_Consensus_Find_Conserved_Region(tk.Frame):

# Create a class variable that can be used in other classes:
    position_pair = []

    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.new_file_list=[]
# Show page title on label:
        ...
        self.label2.pack()
# Create an entry box that enables users to type in the name of the consensus sequence:
        self.entry= tk.Entry(self)
        self.entry.pack()

        self.label3=tk.Label(self, text='Type in MSA threshold (Default is 0.7, meaning 70%)',font=WelcomePage.appNormalFont)
        self.label3.pack()
# Create another entry box that enables users to type in the threshold of the consensus sequence:
        ...
        self.button1 = tk.Button(self,text='Browse',command=self.create_consensus_seq_file,font=WelcomePage.appButtonFont)
        self.button1.pack()

        self.label5 = tk.Label(self, text='Create conserved region file:',font=WelcomePage.appNormalFont)
        self.label5.pack()

        self.button2 = tk.Button(self,text='Browse',command=self.create_conserved_region_file,font=WelcomePage.appButtonFont)
        self.button2.pack()
    ...
    def create_consensus_seq_file(self):
    ...
    def create_conserved_region_file(self):
...
class Design_Primers(tk.Frame):
    ...
    def __init__(self,parent,controller):
        ...
        self.button1 = tk.Button(self,text='Browse',command=self.create_parameters_file,font=WelcomePage.appButtonFont)
        self.button1.pack()

        self.label3 = tk.Label(self, text='Create primer files:',font=WelcomePage.appNormalFont)
        self.label3.pack()

        self.button2 = tk.Button(self,text='Browse',command=self.create_primers_file,font=WelcomePage.appButtonFont)
        self.button2.pack()

        self.label9=tk.Label(self)
        self.label9.pack()

        self.button3 = tk.Button(self, text="Show primers' quality", command=self.create_graph,font=WelcomePage.appButtonFont)
        self.button3.pack()
...
# Create empty lists that wil be used to store data of primers:
...
# Create a function to delete the redundant primers that are of the same target sequence:
...
# Create a function to read in the primer records and collect their data into the lists created before:
...
# Return the sum of hairpin value and dimer value for the comparison bewteen different reocrds in the next function:
        return self.data_sumup

# Create a function to reorder the primer records according to their data:
...
# Read in the parameters in para.txt file:
    def Getpara(self):
        with open('para.txt') as para:
            self.alllines = para.readlines()
            for self.para_index, self.para_line in enumerate(self.alllines):
                ...
                if 'PRODUCT_SIZE_RANGE' in self.para_line:
                    ...
                    if self.product_size[-1] == '\n':
                        ...
                        pass
# Call primer3 in the same directory and run it to generate primers:
                ...
                with open(self.parameters_file, "a+") as para:
                    ...
                    for pair in range(0, len(Generate_Consensus_Find_Conserved_Region.position_pair)):
                        ...
                        para.write("SEQUENCE_ID=" + str(self.template_id) +
                                   ...
                                   'PRIMER_NUM_RETURN=1' + '\n' +
# Use the parameter read from Getpara file:
...
# Make sure that all primers are generated for each input sequences:
...
# Empty the lists again to aviod the accumultion of data when continuously using the pipeline on multiple input files:
            self.primer_tm_list = []
            self.left_primer_tm_list = []
            self.right_primer_tm_list = []
            self.primer_gc_list = []
...
# Create Excel file for the primer result:
            ...
            self.textbox.insert(INSERT,'Program finished, you can get your results in'+ self.primers_file+'\n')
        else:
            self.textbox.insert(INSERT,'Some primers are not generated!'+'\n')
# Create graph to show the quality of the primers with regards to Tm value, GC content, dimer and hairpin value:
    def create_graph(self):
        import matplotlib.pyplot as plt
        import matplotlib
        plt.plot(range(len(self.geneid_list)),self.primer_tm_list,'xkcd:coral',label='Tm')
        plt.plot(range(len(self.geneid_list)),self.primer_gc_list,'xkcd:blue',label='GC%')
        plt.plot(range(len(self.geneid_list)),self.primer_hairpin_list,'xkcd:gold',label='Hairpin')
        plt.plot(range(len(self.geneid_list)),self.primer_self_dimer_list,'xkcd:green',label='Self dimer')
        ...
        plt.clf()
        self.textbox.insert(INSERT, 'You can get the graph of the quality of your primers in' + self.primers_file + '\n')
# Execute the GUI and keep it running untill it is closed manually:
app = PrimerDesigner()
